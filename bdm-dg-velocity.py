...
for deg in degs:
	for N in n_vals:
		...
		v_q = 3
		w_q = 2
		V = FunctionSpace(mesh, "BDM", deg) # velocity
		W = FunctionSpace(mesh, "DG", deg-1) # pressure
		...
		print(f'FEM is BDM{deg} - DG{deg-1}')
		print(f'n = {N}')
...
x0 = norm_pres[0][0]
y0 = n_vals[0]
r_line = 0.5 * x0 * (np.asarray(n_vals)/y0)**-1
plt.loglog(n_vals, r_line, linestyle='--' , label='O(h^1)', color='black')
# second reference line
x1 = norm_pres[1][0]
r_line2 = 0.5 * x1 * (np.asarray(n_vals)/y0)**-2
plt.loglog(n_vals, r_line2, linestyle='-.', label='O(h^2)', color='black')
# third reference line
x2 = norm_pres[2][0]
r_line3 = 0.3*x2*(np.asarray(n_vals)/y0)**-3
plt.loglog(n_vals, r_line3, linestyle=':', label='O(h^3)', color='black')

# Error plots
plt.loglog(n_vals, norm_pres[0], marker='o', label='BDM1-DG0')
plt.loglog(n_vals, norm_pres[1], marker='s', label='BDM2-DG1')
plt.loglog(n_vals, norm_pres[2], marker='^', label='BDM3-DG2')
